...
NODE_SELECTED_COLOUR = gtk.gdk.color_parse("red")
NODE_COMPLETE_COLOUR = gtk.gdk.color_parse("green")
NODE_INCOMPLETE_COLOUR = gtk.gdk.color_parse("yellow")
...
class PathwayDisplay(gtk.VBox):
    ...
    def __init__(self):
        ...
        #self._btn_save_name.connect('clicked', self._click_save_name_cb)  #handling in pathwayview
    ...
    def set_cursor(self, node):
        ...
        if type(node) is ChallengeNode:
            challenge_text = node.challenge.get_description()
            self.btn_add_before.set_sensitive(True)
            self.btn_add_after.set_sensitive(True)
    ...
    def _add_nodes_between(self, box, start, end):
        ...
        while next is not end:
            box.pack_start(self._build_node(next), True, False, 5)
            next = next.after
