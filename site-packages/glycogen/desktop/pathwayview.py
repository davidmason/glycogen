...
class PathwayView(gtk.VBox):
    ...
    def __init__(self, **kwargs):
        ...
        self.challenge_list.connect('cursor-changed', self.challenge_selected_cb)
    ...
    def activity_selected_cb(self, widget, data=None):
        bundle_id = self.get_selected_activity()
        self.challenge_list.show_challenges(bundle_id)
        
    def challenge_selected_cb(self, widget, data=None):
        bundle_id = self.get_selected_activity()
        
        #get selected challenge
        selection = self.challenge_list.get_selection()
        tree_model, tree_iter = selection.get_selected()
        challenge_id = tree_model.get_value(tree_iter, 0)
        
        repository = challrepo.get_global_repository()
        challenge = repository.get_challenge(bundle_id, challenge_id)
        #challenges = repository.get_challenges(bundle_id)
        
        #debugstr = ""
        #for chalid in challenges.keys():
        #    debugstr = debugstr + chalid + "  "
        
        self.debug(str(challenge.get_id()) + str(challenge.get_description()))
        
        self.pathway_display.add_challenge(challenge)
