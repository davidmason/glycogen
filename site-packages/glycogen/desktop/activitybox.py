...
from glycogen.desktop import activitylayout

_logger = logging.getLogger('FavoritesView')

_ICON_DND_TARGET = ('activity-icon', gtk.TARGET_SAME_WIDGET, 0)

LAYOUT_MAP = {#activitylayout.RingActivityLayout.key: activitylayout.RingActivityLayout,
              activitylayout.RandomActivityLayout.key: activitylayout.RandomActivityLayout}
"""Map numeric layout identifiers to uninstantiated subclasses of
`ActivityLayout` which implement the layouts.  Additional information
about the layout can be accessed with fields of the class."""


class ActivityBox(hippo.Canvas):
    __gtype_name__ = 'GlycogenActivityView'

    def __init__(self, **kwargs):
        logging.debug('GLYCOGEN: Loading the activity box')
        ...
        self._box.append(self._my_icon) # looks like this puts the xo user icon in the middle

        self._current_activity = CurrentActivityIcon()
        self._box.append(self._current_activity) # looks like this puts the current activity icon under the user icon
        ...
        logging.warn('GLYCOGEN: setting pathway layout from favorites_settings')
        self._set_layout(favorites_settings.layout)

    def __settings_changed_cb(self, **kwargs):
        favorites_settings = get_settings()
        logging.warn('GLYCOGEN: setting pathway layout from favorites_settings in callback')
        self._set_layout(favorites_settings.layout)
    ...
    def __connect_to_bundle_registry_cb(self):
        ...
        for info in registry:
            #FIXME removed check for whether activities are favorites
    ...
    def __activity_added_cb(self, activity_registry, activity_info):
        #registry = bundleregistry.get_registry()
        #FIXME removed check for whether bundle is in favorites
    ...
    def __activity_changed_cb(self, activity_registry, activity_info):
        ...
        #FIXME removex check for whether activity is in favorites
        #registry = bundleregistry.get_registry()
        #if registry.is_bundle_favorite(activity_info.get_bundle_id(),
        #                               activity_info.get_activity_version()):
        self._add_activity(activity_info)
    ...
    def _set_layout(self, layout):
        if layout not in LAYOUT_MAP:
            logging.warn('GLYCOGEN: Unknown pathway layout: %r' % layout)
            layout = activitylayout.RandomActivityLayout.key
...
class ActivityIcon(CanvasIcon):
    __gtype_name__ = 'GlycogenActivityIcon'
...
class FavoritePalette(ActivityPalette):
    __gtype_name__ = 'GlycogenActivityPalette'
...
class OwnerIcon(BuddyIcon):
    __gtype_name__ = 'GlycogenActivityOwnerIcon'
...
class FavoritesSetting(object):
    ...
    _GLYCOGEN_KEY = "/desktop/sugar/desktop/pathway_layout"

    def __init__(self):
        client = gconf.client_get_default()
        self._layout = client.get_string(self._GLYCOGEN_KEY)
    ...
    def set_layout(self, layout):
        ...
        if layout != self._layout:
            ...
            client = gconf.client_get_default()
            client.set_string(self._GLYCOGEN_KEY, layout)
